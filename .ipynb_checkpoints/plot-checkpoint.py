...
from dataset.dataset_survival_egmde import DataGeneratorTCGASurvivalWSIEGMLL
...
def plot_risk_stratified_cohort(high_risk_data, low_risk_data, p_value, save_path=None):
    ...
    # plt.xlim(0, xmax)
    raw_xmax = max([t for t, _ in high_risk_data + low_risk_data], default=100)
    xmax = int(np.ceil(raw_xmax / 40.0) * 40)  # round up to nearest 40
    plt.xlim(0, xmax)
    plt.ylim(0, 100)

    x_ticks = np.arange(0, xmax + 40, 40)
...
def main(args, cfg, save_dir, fold, save_distribution_data=True):
    ...
    model.to(cfg.device)
    model.load_state_dict(torch.load(os.path.join(save_dir, 'weights', 'epoch_2.pt')))
    model.eval()
